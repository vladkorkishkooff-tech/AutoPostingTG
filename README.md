...
Эти фичи зафиксированы и будут реализованы после завершения текущего функционала:

- [ ] **Редактирование перед публикацией из превью генератора** — поправить текст сгенерированного поста прямо в генераторе Mini App до нажатия «Опубликовать» (базовое редактирование постов очереди уже доступно на экране «Очередь»).
- [ ] **Уведомления в личку** — бот пишет владельцу в личный чат: «Пост опубликован», «Ошибка: все провайдеры недоступны», «Ключ провайдера исчерпан». Контроль системы без захода в приложение.
...
- Статистика: динамика подписчиков, активность постинга, разбивка по темам, использование провайдеров.
